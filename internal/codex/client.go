...
// SetNotificationHandler installs the app-server notification sink used by a
// private session. Callers set it before Start so the first turn lifecycle
// notification cannot be missed.
func (c *Client) SetNotificationHandler(handler func(map[string]any)) {
	if c == nil {
		return
	}
	c.notificationMu.Lock()
	c.onNotification = notificationHandler(handler)
...
func (c *Client) Start(ctx context.Context) error {
	...
	cmd := processlaunch.Command(c.executable, "app-server", "--stdio")
	...
	result, err := c.request(initCtx, "initialize", map[string]any{
		"clientInfo": map[string]any{"name": "cwapi", "title": "CWapi Codex MCP Relay", "version": buildinfo.Version},
