...
$WailsOutput = @(& (Join-Path $PSScriptRoot 'ensure_v161_validation_wails.ps1'))
...
try {
    if (-not (Test-Path -LiteralPath 'node_modules\.bin\tsc.cmd' -PathType Leaf)) {
        Invoke-Checked 'frontend-npm-install' { & $NpmPath ci --prefer-offline --no-audit --no-fund }
    }
} finally {
    Pop-Location
}
...
Invoke-Checked 'wails-windows-build' { & $WailsPath build -clean -skipbindings -trimpath -platform windows/amd64 -ldflags $LdFlags }
...
$ProbeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("cwapi-v161-packaged-gui-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($Required in @('first-run-slack', 'version-1.6.1')) {
