...
if ($Expectations.schema -ne 'cwapi.slack-mcp-e2e.expectations.v2' -or @($Expectations.requests).Count -lt 1) {
...
try {
    ...
    if ($Evidence.schema -ne 'cwapi.slack-mcp-e2e.result.v2') { throw 'CWAPI_REAL_SLACK_RESULT_SCHEMA_INVALID' }
